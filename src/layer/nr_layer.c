...
/* "Pseudo single DLL": spawn the daemon from inside the layer so the user never has
 * to start it by hand. The game directory then holds only this DLL, the weights, and
 * a Python runtime — the daemon is a sibling script the layer launches on load.
 *
 * Anything the user already set wins: if NR_LAYER_SOCKET names a daemon that is
 * already listening, we do not spawn a second one (a running daemon refuses to
 * start). If NR_DAEMON is set we use it; otherwise we look next to this very module.
 * Failure is non-fatal: without a daemon the frame simply stays as the game drew it,
 * which is the existing degrade path. */
static void ensure_daemon(void)
{
...
#else
	{
		pid_t pid = fork();
		if (pid < 0) {
			fprintf(stderr, "[nr_layer] daemon spawn failed (fork); frame stays "
				"as the game drew it\n");
			return;
		}
		if (pid == 0) {
			...
			execlp(py, py, daemon, "--socket", sock, "--settings", settings,
			       "--root", root, (char *)NULL);
			_exit(127);
		}
	}
						 ...
						 VkInstance *instance)
{
	...
	if (r == VK_SUCCESS) {
		...
		/* Pseudo single DLL: bring the daemon up ourselves if it is not already
		 * running, so the user only ships this DLL, the weights and a Python. */
		ensure_daemon();
	}
	return r;
}
